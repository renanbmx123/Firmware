...
int main(){
  ...
  //bool data_send = false; // Send data flag, for tell the deive to sleep if data is sended.
  ...
  float       g[3],       // Gyroscope
  acc[3]      // Accelerometer
  ,mag[3],    // Magnetrometer //36
  dht_t,      // DHT22 temperature sensor
  dht_h,      // DHT22 humidity sensro
  piezo,      // Vibration sensor
  vbat,       // Bateria percentage.
  lat,        // Latitude .
  lon,        // Longitude.
  ...
  while(1){
    ...
    */
    for (int j = 0; j < 10; j++){
        ...
        if(tgps.NorthSouth == 'S' | tgps.NorthSouth =='N'){
          NorthSouth = tgps.NorthSouth;
          }
        else{
          NorthSouth = 'F';
        }
        if (tgps.EastWest == 'W' | tgps.EastWest == 'E'){
        ...
        {
          date[0] = tgps.time.hour();
          date[1] = tgps.time.minute();
          date[2] = tgps.time.second();
        }else{
          date[0] = 0;
          date[1] = 0;
          date[2] = 0;
      ...
      g[0], g[1], g[2],acc[0],acc[1], acc[2], mag[0], mag[1], mag[2],Bpress, dht_h,dht_t,MQ4_data.ch4,piezo, date[0],date[1],date[2], lon, lat, speed, NorthSouth, EastWest, Balt, alt,vbat);
